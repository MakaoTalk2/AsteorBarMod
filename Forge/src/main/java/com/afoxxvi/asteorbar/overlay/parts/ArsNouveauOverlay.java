...
public class ArsNouveauOverlay extends SimpleBarOverlay {
    ...
    protected void drawDecorations(PoseStack poseStack, int left, int top, int right, int bottom, Parameters parameters, boolean flip) {
        ...
        if (ClientInfo.reservedOverlayMana > 0.0F) {
            ...
            drawFillFlipConcat(poseStack, left + 1, top + 1, right - 1, bottom - 1, offset, reserveManaLength, 0xff330561, flip);
        }
        if (ClientInfo.redTicks()) {
            ...
            drawFillFlip(poseStack, left + 1, top + 1, right - 1, bottom - 1, 0, redManaLength, color, flip);
        }
    }
