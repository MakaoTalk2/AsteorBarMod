...
public class HomeostaticOverlay extends SimpleBarOverlay {
    ...
    protected void drawDecorations(GuiGraphics guiGraphics, int left, int top, int right, int bottom, Parameters parameters, boolean flip) {
        ...
        drawFillFlipConcat(guiGraphics, left + 1, top + 1, right - 1, bottom - 1, (int) fillWidth, (int) ((right - left - 2) * hydrationLevel / parameters.capacity), parameters.fillColor, flip);
        double boundWidth = (right - left) * parameters.boundValue / parameters.boundCapacity;
        drawBoundFlipConcat(guiGraphics, left, top, right, bottom, (int) boundWidth, (int) ((right - left) * hydrationSaturationLevel / parameters.boundCapacity), parameters.boundFillColor, flip);
