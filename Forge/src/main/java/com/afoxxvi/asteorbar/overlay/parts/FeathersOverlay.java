...
public class FeathersOverlay extends SimpleBarOverlay {
    ...
    protected void drawDecorations(GuiGraphics guiGraphics, int left, int top, int right, int bottom, Parameters parameters, boolean flip) {
        ...
        drawFillFlip(guiGraphics, left + 1, top + 1, right - 1, bottom - 1, fillWidth, 0xffb8b9c4, flip);
        if (times > 0) {
            if (flip) {
        ...
        if (regenerationOffset >= 0) {
            ...
            if (textureRight > 0) {
                drawTextureFillColor(guiGraphics, left + 1, top, -textureLeft, 5, 10 + 180 + textureLeft, Y_REGENERATION_FILL, -textureLeft, 5, 0xffbedcfa);
                drawTextureFillColor(guiGraphics, left + 1 - textureLeft, top, textureRight, 5, 10, Y_REGENERATION_FILL, textureRight, 5, 0xffbedcfa);
            } else {
                drawTextureFillColor(guiGraphics, left + 1, top, right - left - 2, 5, 10 + 180 + textureLeft, Y_REGENERATION_FILL, right - left - 2, 5, 0xffbedcfa);
            }
