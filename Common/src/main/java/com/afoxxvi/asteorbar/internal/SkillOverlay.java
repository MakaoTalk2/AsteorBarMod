...
public class SkillOverlay extends BaseOverlay {
    ...
    public void renderOverlay(RenderGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        RenderSystem.setShaderTexture(0, new ResourceLocation("minecraft", "textures/gui/sprites/hud/hotbar_offhand_left.png"));
        int middle = screenWidth / 2;
        for (int i = 0; i < 4; i++) {
            int x = middle - 91 - 29 - 21 * 4 + i * 21;
            int y = screenHeight - 23;
            GuiHelper.drawTexturedRect(guiGraphics, x, y, 0, 0, 29, 24, 29, 24);
        }
        RenderSystem.setShaderTexture(0, new ResourceLocation("minecraft", "textures/gui/sprites/hud/hotbar_offhand_right.png"));
        for (int i = 0; i < 8; i++) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (!texture.isEmpty()) {
                ...
                GuiHelper.drawTexturedRect(guiGraphics, x, y, x + 16, y + 16, 0, 0, 32, 32, 32, 32);
            }
            ...
            if (AsteorBar.tick - InternalInfo.skillGlint[i] < 20) {
                ...
                GuiHelper.drawSolidColor(guiGraphics, x, y, x + 16, y + 16, color);
            }
            ...
            if (cooldown < Byte.MAX_VALUE) {
                ...
                GuiHelper.drawSolidColor(guiGraphics, x + 8, y, x + 16, y + 16, 0x80D0D0D0);
                GuiHelper.drawSolidColor(guiGraphics, x + 8, y + len, x + 16, y + 16, 0x80D0D0D0);
            ...
            if (mana < Byte.MAX_VALUE) {
                ...
                GuiHelper.drawSolidColor(guiGraphics, x, y, x + 8, y + 16, 0x8000FFFF);
                GuiHelper.drawSolidColor(guiGraphics, x, y + len, x + 8, y + 16, 0x8000FFFF);
