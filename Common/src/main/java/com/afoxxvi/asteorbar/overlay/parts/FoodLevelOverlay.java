...
public class FoodLevelOverlay extends BaseOverlay {
    ...
    private void draw(PoseStack poseStack, int left, int top, int right, int bottom, boolean highlight, int foodColor, int foodLevel, float saturation, float exhaustion, int foodIncrement, float saturationIncrement, int tick, boolean flip) {
        ...
        if (foodIncrement > 0 && foodLevel < AsteorBar.config.fullFoodLevelValue()) {
            ...
            drawFillFlipConcat(poseStack, left + 1, top + 1, right - 1, bottom - 1, foodWidth, incrementWidth, foodColor, flip);
        ...
        if (AsteorBar.config.displaySaturation()) {
            ...
            if (foodIncrement > 0 && saturationIncrement > 0 && saturation < AsteorBar.config.fullSaturationValue()) {
                ...
                RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, alpha);
                drawBoundFlipConcat(poseStack, left, top, right, bottom, saturationWidth, incrementWidth, AsteorBar.config.saturationColor(), flip);
                RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
            }
        }
    ...
    public void renderOverlay(RenderGui gui, PoseStack poseStack, float partialTick, int screenWidth, int screenHeight) {
        ...
        draw(poseStack, left, top, right, top + 5, foodBlinkTime > 0, foodType, level, saturation, exhaustion, foodIncrement, saturationIncrement, gui.gui().getGuiTicks(), flip);
    }
}
